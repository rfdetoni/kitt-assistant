...
const state={catalog:null,snapshot:null,pending:new Map(),section:null,csrf:null};
...
function bindInputs(){document.querySelectorAll("[data-key]").forEach(el=>el.addEventListener("change",()=>{const section=state.catalog.sections.find(s=>s.id===el.dataset.section),field=section.fields.find(f=>f.key===el.dataset.field);let value=el.type==="checkbox"?el.checked:el.value;if(field.type==="integer")value=Number.parseInt(value,10);if(field.type==="number")value=Number(value);if(field.type==="string_list")value=value.split(",").map(v=>v.trim()).filter(Boolean);state.pending.set(el.dataset.key,value);render();}));}
